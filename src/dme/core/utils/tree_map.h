...
namespace dme {
	//平衡二叉树实现的指针稳定的Map，类型需支持小于比较
	//直接修改key的行为未定义
	template<typename K, typename V, typename LessFunc = std::less<K>> requires IsInvocableRet<LessFunc, bool, const K&, const K&>
	class TreeMap {
	public:
		using mem_type = core::_tree_pool_mem<true, K, V, LessFunc>;
		...
		using iterator = core::_tree_iterator<TreeMap>;
		using const_iterator = core::_tree_const_iterator<TreeMap>;
	private:
		mem_type mem;
	public:

		template<typename F = LessFunc> requires std::same_as<std::remove_cvref_t<F>, LessFunc>
		constexpr TreeMap(F&& f = F()) noexcept : mem(std::forward<F>(f)) {}

		template<typename F = LessFunc> requires std::same_as<std::remove_cvref_t<F>, LessFunc>
		constexpr explicit TreeMap(size_t size, F&& f = F()) noexcept : mem(size, std::forward<F>(f)) {}

		constexpr TreeMap(const TreeMap& a) noexcept(mem_type::noexcept_copy) : mem(a.mem) {}
		constexpr TreeMap(TreeMap&& a) noexcept : mem(std::move(a.mem)) {}


		template<typename F = LessFunc> requires std::same_as<std::remove_cvref_t<F>, LessFunc>
		constexpr TreeMap(std::initializer_list<value_type> list, F&& f = F()) noexcept(mem_type::noexcept_copy) : mem(list, std::forward<F>(f)) {}

		constexpr TreeMap& operator =(const TreeMap& a) noexcept(mem_type::noexcept_copy) {
			mem = a.mem;
			return *this;
		}
		constexpr TreeMap& operator =(TreeMap&& a) noexcept {
			mem = std::move(a.mem);
			return *this;
		}
		constexpr TreeMap& operator =(std::initializer_list<value_type> list) noexcept(mem_type::noexcept_copy) {
			mem = list;
			return *this;
		}
		//返回起点的迭代器
		...
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* add(KT&& key_, VT&& val_) noexcept(noexcept(mem.add(std::forward<KT>(key_), std::forward<VT>(val_)))) {
			return mem.add(std::forward<KT>(key_), std::forward<VT>(val_));
		}

		//不存在则添加元素，已存在则替换val，返回新加元素或已有元素的指针
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* add(Pair<KT, VT>&& pair) noexcept(noexcept(mem.add(std::forward<KT>(pair.key), std::forward<VT>(pair.val)))) {
			return mem.add(std::forward<KT>(pair.key), std::forward<VT>(pair.val));
		}

		//不存在则添加元素，已存在则替换
		constexpr void add(const TreeMap& other) noexcept(noexcept(mem.addMem(other.mem))) {
			mem.addMem(other.mem);
		}
		//不存在则添加元素，已存在则替换
		constexpr void add(TreeMap&& other) noexcept(noexcept(mem.addMem(std::move(other.mem)))) {
		...
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* addIfAbsent(KT&& key_, VT&& val_) noexcept(noexcept(mem.addIfAbsent(std::forward<KT>(key_), std::forward<VT>(val_)))) {
			return mem.addIfAbsent(std::forward<KT>(key_), std::forward<VT>(val_));
		}

		//如果不存在则添加元素，返回新加元素或已有元素的指针
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* addIfAbsent(Pair<KT, VT>&& pair) noexcept(noexcept(mem.addIfAbsent(std::forward<KT>(pair.key), std::forward<VT>(pair.val)))) {
			return mem.addIfAbsent(std::forward<KT>(pair.key), std::forward<VT>(pair.val));
		}

		//添加所有非重复元素
		constexpr void addIfAbsent(const TreeMap& other) noexcept(noexcept(mem.addMemIfAbsent(other.mem))) {
			mem.addMemIfAbsent(other.mem);
		}
		//添加所有非重复元素
		constexpr void addIfAbsent(TreeMap&& other) noexcept(noexcept(mem.addMemIfAbsent(std::move(other.mem)))) {
			mem.addMemIfAbsent(std::move(other.mem));
		}

		//替换val，不存在则返回null
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* replace(KT&& key_, VT&& val_) noexcept(noexcept(mem.replace(std::forward<KT>(key_), std::forward<VT>(val_)))) {
			return mem.replace(std::forward<KT>(key_), std::forward<VT>(val_));
		}

		//替换val，不存在则返回null
		template <typename KT = K, typename VT = V> requires(IsSameOrConvertible<KT, K>&& IsSameOrConvertible<VT, V>)
			constexpr value_type* replace(Pair<KT, VT>&& pair) noexcept(noexcept(mem.replace(std::forward<KT>(pair.key), std::forward<VT>(pair.val)))) {
			return mem.replace(std::forward<KT>(pair.key), std::forward<VT>(pair.val));
		}

		//替换key，不存在则返回null
		template <typename KT1 = K, typename KT2 = K> requires(IsSameOrConvertible<KT1, K>&& IsSameOrConvertible<KT2, K>)
			constexpr value_type* replaceKey(KT1&& oldKey_, KT2&& newKey_) noexcept(noexcept(mem.replaceKey(std::forward<KT1>(oldKey_), std::forward<KT2>(newKey_)))) {
			return mem.replaceKey(std::forward<KT1>(oldKey_), std::forward<KT2>(newKey_));
		}
		...
		//移除之前add返回的指针上的元素
		constexpr bool remove(value_type* key) noexcept {
			return mem.removeByValuePtr(key);
		}
		...
		//遍历元素
		constexpr void each(const std::function<void(value_type&)>& func) const noexcept(noexcept(mem.each(func))) {
			mem.each(func);
		}
		//遍历元素，传入函数的第二个参数如果修改为true，则跳过剩下的元素直接结束
		constexpr void each(const std::function<void(value_type&, bool&)>& func) const noexcept(noexcept(mem.each(func))) {
			mem.each(func);
		}
		...
		constexpr bool operator ==(const TreeMap& a) const noexcept {
			return mem == a.mem;
		}
		friend std::ostream& operator<<(std::ostream& os, const TreeMap& a) {
			return os << a.mem;
		}
		friend constexpr Hash Hasher(const TreeMap& a) noexcept {
			return Hasher(a.mem);
		}
	};
...
namespace std {
	...
	struct hash<dme::TreeMap<K, V>> {
		size_t operator ()(const dme::TreeMap<K, V>& a) const noexcept {
			return Hasher(a);
		}
	};
}