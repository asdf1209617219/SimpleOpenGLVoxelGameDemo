...
namespace dme {
	//flat_hash实现的key不具有指针稳定性但是val具有指针稳定性的Map，类型需支持哈希和等于比较
	//直接修改key的行为未定义
	template<typename K, typename V> requires (IsCanHash<K> && IsCanEqual<K>)
	class FlatKeyMap {
	public:
		using mem_type = core::_flat_hash_mem<true, K, V, false, true>;
		...
		using iterator = core::_flat_hash_mem_iterator<FlatKeyMap>;
		using const_iterator = core::_flat_hash_mem_const_iterator<FlatKeyMap>;
	private:
		mem_type mem;
	public:
		constexpr FlatKeyMap() noexcept : mem() {}
		constexpr explicit FlatKeyMap(size_t size) noexcept : mem(size) {}
		constexpr FlatKeyMap(const FlatKeyMap& a) noexcept(mem_type::noexcept_copy) : mem(a.mem) {}
		constexpr FlatKeyMap(FlatKeyMap&& a) noexcept : mem(std::move(a.mem)) {}
		constexpr FlatKeyMap(std::initializer_list<value_type> list) noexcept(mem_type::noexcept_copy) : mem(list) {}
		constexpr FlatKeyMap& operator =(const FlatKeyMap& a) noexcept(mem_type::noexcept_copy) {
			mem = a.mem;
			return *this;
		}
		constexpr FlatKeyMap& operator =(FlatKeyMap&& a) noexcept {
			mem = std::move(a.mem);
			return *this;
		}
		constexpr FlatKeyMap& operator =(std::initializer_list<value_type> list) noexcept(mem_type::noexcept_copy) {
			mem = list;
			return *this;
		}
		...
		constexpr bool operator ==(const FlatKeyMap& a) const noexcept {
			return mem == a.mem;
		}
		friend std::ostream& operator<<(std::ostream& os, const FlatKeyMap& a) {
			return os << a.mem;
		}
		friend constexpr Hash Hasher(const FlatKeyMap& a) noexcept {
			return Hasher(a.mem);
		}
	};
}